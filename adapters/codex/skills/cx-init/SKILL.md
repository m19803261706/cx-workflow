---
name: cx-init
description: "Codex 侧 CX 项目初始化。收集项目级配置，建立 开发文档/CX工作流 与 .cx，并确保共享 cx core 可立即供 CC 与 Codex 共用。"
---
...
1. 检测项目根目录与 Git 状态
2. 检测 `开发文档/CX工作流` 或旧版 `.claude/cx` 是否已存在
3. 如果已经有 `.cx/core/projects/*.json`
   - 只做健康检查与缺失修复，不重复初始化
4. 如果存在旧版 `开发文档/CX工作流/配置.json` / `开发文档/CX工作流/状态.json` 但没有 `core/`
...
- 项目里有 `开发文档/CX工作流/配置.json`
- 项目里有 `开发文档/CX工作流/状态.json`
- 项目里有 `.cx/core/projects/project.json`
...
- 不要把 Codex 运行时文件写到 `.cx/runtime/cc/`
