...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  if command -v osascript &>/dev/null; then
    osascript -e "display notification \"$MSG\" with title \"cx\"" &>/dev/null || true
  fi

elif [[ -n "${WSL_DISTRO_NAME:-}" ]] || [[ -n "${WSL_INTEROP:-}" ]]; then
  # WSL (Windows Subsystem for Linux)
  if command -v powershell.exe &>/dev/null; then
    powershell.exe -Command "[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); [System.Windows.Forms.MessageBox]::Show('$MSG','cx')" &>/dev/null &
  elif command -v notify-send &>/dev/null; then
    # Fallback to notify-send on WSL with notification server
    notify-send "cx" "$MSG" &>/dev/null || true
  fi
